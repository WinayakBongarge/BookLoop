...
const ProfileSection: React.FC<ProfileSectionProps> = ({ title, children }) => (
    <div className="bg-surface rounded-lg shadow-md p-6">
        <h3 className="text-xl font-semibold border-b pb-3 mb-4">{title}</h3>
        {children}
    </div>
);
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                <div className="lg:col-span-1 space-y-6">
                    <div className="bg-surface rounded-lg shadow-md p-6 text-center">
                        ...
                        <h2 className="text-2xl font-bold">Rohan Gupta</h2>
                        <p className="text-gray-500">rohan.gupta@example.com</p>
                        <Button variant="secondary" className="mt-4 !text-sm !px-4 !py-2" onClick={() => handleAction('Uploading new picture...')}>Upload Picture</Button>
                    </div>
                    <ProfileSection title="Location">
                        <p className="text-gray-600">42, MG Road, Sector 18<br/>Noida, Uttar Pradesh - 201301</p>
                        <Button variant="secondary" className="w-full mt-4" onClick={() => handleAction('Changing location...')}>Change Location</Button>
                    </ProfileSection>
                </div>
                     ...
                     <ProfileSection title="Personal Information">
                        <div className="space-y-4">
                            ...
                            <div className="text-right pt-2">
                                <Button onClick={() => handleAction('Saving changes...')}>Save Changes</Button>
                            </div>
                        </div>
                    </ProfileSection>
                    <ProfileSection title="Security">
                         <div className="space-y-4">
                            ...
                            <InputField label="Confirm New Password" type="password" placeholder="••••••••" />
                             <div className="text-right pt-2">
                                <Button onClick={() => handleAction('Updating password...')}>Update Password</Button>
                            </div>
                         </div>
                    </ProfileSection>
                    <div className="flex justify-between items-center mt-6 p-4 bg-red-50 rounded-lg border border-red-200">
                        <Button variant="danger" onClick={() => handleAction('Logging out...')}>Log Out</Button>
                        <button onClick={() => handleAction('Deleting account...')} className="text-sm text-red-600 hover:underline font-semibold">Delete Account</button>
                    </div>
                </div>
            </div>
        </div>
